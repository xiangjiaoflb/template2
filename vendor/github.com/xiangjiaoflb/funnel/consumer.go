...
func (c *Consumer) Start(inputStream io.Reader) error {
	log.SetFlags(log.Lshortfile | log.LstdFlags)
	//c.setupSignalHandling()

	c.lastRollover = time.Now()
...
outer:
	for {
		...
		default:
			...
			//打印到标准输出
			if c.Config.WriteOutput {
				log.Printf("%s %s", c.Config.ActiveFileName, line)
			}
...
func (c *Consumer) rollOverCondition() bool {
	...
	return c.linesWritten >= c.Config.RotationMaxLines ||
		...
		c.lastRollover.Day() != time.Now().Day()
}
...
// HandleSignal ...
func (c *Consumer) HandleSignal(s os.Signal) {
	switch s {
